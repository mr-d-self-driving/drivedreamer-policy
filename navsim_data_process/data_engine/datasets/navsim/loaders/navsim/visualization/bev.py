...
def add_configured_bev_on_ax(ax: plt.Axes, ann) -> plt.Axes:
    """
    ...
    if "annotations" in BEV_PLOT_CONFIG["layers"]:
        add_annotations_to_bev_ax(ax, ann, add_ego=False)
...
def add_configured_bev_on_ax_ori(ax: plt.Axes, map_api, frame) -> plt.Axes:
    """
    Adds birds-eye-view visualization optionally with map, annotations, or lidar
    :param ax: matplotlib ax object
    :param map_api: nuPlans map interface
    ...
    if "annotations" in BEV_PLOT_CONFIG["layers"]:
        add_annotations_to_bev_ax_ori(ax, frame.annotations, add_ego=False)
...
def add_annotations_to_bev_ax(ax: plt.Axes, annotations: Annotations, add_ego: bool = True) -> plt.Axes:
    ...
    for name_value, box_value in zip(annotations['gt_names'], annotations['gt_boxes']):
        agent_type = tracked_object_types[name_value]

        if name_value not in ['vehicle', 'pedestrian', 'bicycle']:
            continue
        ...
        add_oriented_box_to_bev_ax(ax, agent_box, AGENT_CONFIG[agent_type], add_heading=False)
...
def add_annotations_to_bev_ax_ori(ax: plt.Axes, annotations: Annotations, add_ego: bool = True) -> plt.Axes:
    """
    Adds birds-eye-view visualization of annotations (ie. bounding boxes)
    :param ax: matplotlib ax object
    :param annotations: navsim annotations dataclass
