...
from .bev import add_configured_bev_on_ax, add_trajectory_to_bev_ax, add_configured_bev_on_ax_ori
...
def configure_bev_ax(ax: plt.Axes) -> plt.Axes:
    ...
    # ax.set_xlim(-10, margin_y-10)
    # ax.set_ylim(-margin_x / 2, margin_x / 2)
    ax.set_ylim(0, margin_x)
...
def plot_bev_with_agent(anns, agent: AbstractAgent) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    BEV_PLOT_CONFIG['figure_margin'] = (40, 40)
    fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    add_configured_bev_on_ax(ax, anns)
...
def plot_bev_with_agent_ori(scene, frame_idx) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
    ...
    # frame_idx = scene.scene_metadata.num_history_frames - 1
    BEV_PLOT_CONFIG['figure_margin'] = (64, 64)
    fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    add_configured_bev_on_ax_ori(ax, scene.map_api, scene.frames[frame_idx])
    # add_trajectory_to_bev_ax(ax, human_trajectory, TRAJECTORY_CONFIG["human"])
    # add_trajectory_to_bev_ax(ax, agent_trajectory, TRAJECTORY_CONFIG["agent"])
    configure_bev_ax(ax)
    configure_ax(ax)
