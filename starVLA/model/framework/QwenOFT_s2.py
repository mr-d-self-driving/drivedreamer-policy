...
# for preference learing
@FRAMEWORK_REGISTRY.register("QwenOFT_s2")
class Qwenvl_OFT_s2(baseframework):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.preference_token = ["<robot_history_action_0>"]*4
        self.pos_preference_query = nn.Parameter(torch.randn(4, self.qwen_vl_interface.model.config.hidden_size) * 0.02)
        self.neg_preference_query = nn.Parameter(torch.randn(4, self.qwen_vl_interface.model.config.hidden_size) * 0.02)
    ...
    def forward(
        ...
        **kwargs,
        ...
        ori_instructions = [example["lang"] for example in examples]  # [B, str]
        try:
        ...
        instructions = []
        for mode, instruction in zip(infer_mode, ori_instructions):
            hist_str  = self.robot_history_token
            # add preference token
            if mode !=0:
                preference = "".join(self.preference_token)
            ...
            if mode !=0:
                suffix = f" {hist_str}{preference}{rgb_str}{gs_str}{act_str}{rew_str}"
            else:
                suffix = f" {hist_str}{rgb_str}{gs_str}{act_str}{rew_str}"
            instructions.append(instruction + suffix)
        ...
        if self.config.datasets.vla_data.load_act_data:
            for b in range(B):
                ...
                if infer_mode[b] == 0:
                    continue
                if infer_mode[b] !=0:
                    preference_pos = where[1:, 0]
                if infer_mode[b] == 1:
                    text_embeds[b, preference_pos, :] = self.pos_preference_query
                else:
                    text_embeds[b, preference_pos, :] = self.neg_preference_query
        ...
        if self.config.datasets.vla_data.load_act_data == 1:
            ...
            with torch.autocast("cuda", dtype=torch.float32):
                ...
                if self.mlp_head == 0:
                    action_loss = self.action_model(action_queries, actions)  # (B, chunk_len, action_dim)
                else:
        ...
        if self.config.datasets.video_data.load_2d_data:
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                rgb_loss = self.rgb_model(rgb_data, rgb_queries)
            
            return {"action_loss": action_loss, "rgb_loss": rgb_loss}
        else:
